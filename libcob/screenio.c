...
/* work around PDC portability issue returning OK (0) / ERR (-1)
   instead of TRUE (1) and FALSE (0) */
#if !(defined(PDC_BUILD) && (PDC_BUILD >= 4500))
#define panel_hidden(x)   !panel_hidden(x)
#endif
...
static WINDOW		*mywin;		/* current window in use by screenio */
...
typedef struct	__LIST_FLD
	{
		cob_u16_t		pan_position;
		cob_u16_t 		pan_number;
		cob_u16_t 		starty;
		cob_u16_t 		startx;
		cob_u16_t 		win_lines;
		cob_u16_t 		win_cols;
		cob_u16_t		input_fg_color;
		cob_u16_t		input_bg_color;
		cob_u16_t 		flags;
	} LIST_FLD, 	*PLIST_FLD;
...
cob_screen_init (void)
{
	...
	cobglobptr->window_ptr = (void **)&mywin;
...
cob_exit_screen (void)
{
	...
	if (cobglobptr->cob_screen_initialized) {
		if (pending_accept && cobsetptr->cob_exit_wait) {
			...
			WINDOW		*savwin = mywin;
			/* restore main window and its fg+bg color */
			mywin = stdscr;
			fore_color = stdscr_fore_color;
			back_color = stdscr_back_color;
			/* ensure current content is drawn; may not be the case for example
			   if signal handler was executed before */
			refresh ();	
			if (cobsetptr->cob_exit_msg[0] != 0) {
				const size_t	msg_len = strlen(cobsetptr->cob_exit_msg);
				cob_field	field;
				cob_field_attr	attr;
				int		cline = LINES - 2;	/* output at penultimate line */
				int		ccolumn = 1 + (COLS - msg_len) / 2;	/* if one-liner: centered */
				char	*lf_pos = strchr (cobsetptr->cob_exit_msg, '\n');
				if (lf_pos || ccolumn < 0) {
					do {
						if (cline < 2) {
							cline = 1;
						...
						cline--;						
						ccolumn += COLS;
						lf_pos = strchr (lf_pos + 1, '\n');
					} while (lf_pos || ccolumn < 1);
					ccolumn = 0;
				}
				COB_FIELD_INIT (msg_len, (unsigned char *)cobsetptr->cob_exit_msg, &attr);
				COB_ATTR_INIT (COB_TYPE_ALPHANUMERIC, 0, 0, 0, NULL);
				field_display (&field, COB_SCREEN_REVERSE, cline, ccolumn,
			...
			field_accept_from_curpos (NULL, NULL, NULL, NULL,
				NULL, NULL, NULL, COB_SCREEN_NO_ECHO);
			mywin = savwin;
		}
...
cob_sys_set_csr_pos (unsigned char *fld)
{
	...
	if (f && f->size == 4) {
		/* group with sizes up to 64k (2 * 2 bytes)
		   as used by Fujitsu (likely with a limit of
		   254 which does _not_ apply to GnuCOBOL) 
			Note Fujitsu make use of a compiler option
			to set MLBON (Big Endian) or MLBOFF (Little
			Endian)
				So to accomodate this we need to check
				the value of cline / ccol to see if
				either of them is greater than 255 and
				if so to swap them					*/
		...
		cline = bline;
		ccol = bcol;
		if (ccol > 255 || cline > 255) {
			ccol	=	COB_BSWAP_16(ccol);
			cline	=	COB_BSWAP_16(cline);
		}
	} else {
		/* group with sizes up to 255 (2 * 1 bytes)
		   as used by MicroFocus [including C wrappers!]) */
...
sys_window_list (PLIST_FLD list, size_t amount)
{
	...
	/* go through the curses panel deck to add  all the active windows,
	...
	/* go through our internal list to find all of the hidden windows,
	   which are not in the panel deck */
	{
		depth = amount;
		...
		switch (mode) {
			...
			case SYS_WIN_ATTR_CHANGE:
				...
				if (parm->startx > COB_MAX_X_COORD)
					return 4;
				if (parm->starty + parm->win_lines - 1 > COB_MAX_Y_COORD)
					return 6;
				if (parm->startx + parm->win_cols - 1 > COB_MAX_X_COORD)
					return 7;
				if (parm->bg_color > 15)
					return 8;
				if (parm->fg_color > 15)
	...
	if (mode == SYS_WIN_ATTR_CHANGE) {
		...
		if (parm->win_lines != ptr_pan_entry->win_lines
		 ...
		 || parm->starty   != ptr_pan_entry->starty + 1) {
			...
			int local_ret = sys_window_move (parm);
			if (ret == 0) ret = local_ret;
		}
		if (ptr_pan_entry->flags != parm->flags) {
			if ((parm->flags & SYS_WIN_FLAG_HIDDEN)
			 && !(ptr_pan_entry->flags & SYS_WIN_FLAG_HIDDEN)) {
				int local_ret = sys_window_hide (parm);
				if (ret == 0) ret = local_ret;
			}
