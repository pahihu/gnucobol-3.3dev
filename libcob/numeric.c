...
{
	if (cob_last_exception_is (COB_EC_SIZE_ZERO_DIVIDE) 
	&& !opt
	&& COB_GET_LINE_NUM(cobglobptr->cob_current_module->module_stmt)
	&& cobglobptr->last_exception_line 
		== COB_GET_LINE_NUM(cobglobptr->cob_current_module->module_stmt)) {
		cob_runtime_error ("DIVIDE BY ZERO --> termination");
		cob_hard_failure ();
	} 
	if (unlikely (d->scale == COB_DECIMAL_NAN)) {
