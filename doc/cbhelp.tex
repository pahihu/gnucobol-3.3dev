...
@item -M
output dependency list in Makefile format

@item -@code{MT} @var{target}
set/add target file used in dependency list

@item -@code{MQ} @var{target}
same as -@code{MT} but with Makefile-quoting of the target

@item -@code{MF} @var{file}
place dependency list into @var{file}

@item -@code{MP}
create phony targets for all dependencies

@item -@code{MG}
output missing dependencies without complaining

@item -@code{MD}
output dependencies in .d files while compiling

@item -ext @var{extension}
add file extension for resolving @code{COPY}

@item --gentable=@var{ebcdic-enc},@var{ascii-enc}[+]
output a translation table between the
...
default if no scope specified: @code{ALL}
...
@item -fdefault-file-colseq=@code{[ASCII|EBCDIC|NATIVE]}
define default file collating sequence; default: @code{NATIVE}

@item -fwinmain
generate WinMain instead of main when compiling
as executable

@item -fuse-constructor
generate internal one-time code via constructor
...
incorrect order of @code{CONFIGURATION} @code{SECTION} and its paragraphs
